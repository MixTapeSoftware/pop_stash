defmodule PopStash.MCP.Tools.RecallTest do
  ...
  describe "execute/2 - exact match" do
    test "returns insight when exact key match found", %{project: project, agent: agent} do
      {:ok, insight} =
        Memory.create_insight(project.id, agent.id, "JWT uses HS256 by default",
          key: "jwt-config"
        )
  ...
  describe "execute/2 - semantic search fallback" do
    test "returns empty results with hint when no match found", %{project: project, agent: agent} do
      # Create some insights with keys
      {:ok, _} = Memory.create_insight(project.id, agent.id, "Content 1", key: "auth-setup")
      {:ok, _} = Memory.create_insight(project.id, agent.id, "Content 2", key: "db-config")
  ...
  describe "execute/2 - result format" do
    test "formats insight with all expected fields", %{project: project, agent: agent} do
      {:ok, insight} =
